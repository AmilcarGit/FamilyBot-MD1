...
async function responder(ctx) {
  ...
  try {
    ...
    let texto = `👑 ${ctx.chat.title}\n\n👑 ${t(grupo, 'admin')}: ${admins.length}\n\n`;
    if (creador.length) texto += `👑 Propietario\n${creador.map((admin) => `• ${nombreUsuario(admin.user)}`).join('\n')}\n\n`;
    if (administradores.length) texto += `🛡️ Administradores\n${administradores.map((admin) => `• ${nombreUsuario(admin.user)}`).join('\n')}`;
    if (!creador.length && !administradores.length) texto += '• No disponible';
    return enviarMenu(ctx, texto, menu(grupo), null);
  } catch (error) {
