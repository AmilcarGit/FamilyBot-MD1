...
async function mostrar(ctx, editar = false) {
  if (!isGroup(ctx)) return ctx.reply(t({}, 'soloGrupoConfig'));
  const grupo = db.getGrupo(ctx.chat.id);
  const opciones = teclado(grupo);
  const mensaje = estado(grupo);
  if (editar) return ctx.editMessageText(mensaje, opciones);
  return ctx.reply(mensaje, opciones);
}

module.exports = (bot) => {
  bot.command('config', (ctx) => mostrar(ctx));

  bot.action('menu_config', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    return mostrar(ctx, true);
  });

  bot.action('config_bienvenida', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    if (!(await requireAdmin(ctx))) return;
    const grupo = db.getGrupo(ctx.chat.id);
    const actualizado = db.setGrupo(ctx.chat.id, { welcomeEnabled: !grupo.welcomeEnabled });
    return mostrar(ctx, true);
  });

  bot.action('config_despedida', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    if (!(await requireAdmin(ctx))) return;
    const grupo = db.getGrupo(ctx.chat.id);
    db.setGrupo(ctx.chat.id, { goodbyeEnabled: !grupo.goodbyeEnabled });
    return mostrar(ctx, true);
  });

  bot.action('config_antilink', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    if (!(await requireAdmin(ctx))) return;
    const grupo = db.getGrupo(ctx.chat.id);
    db.setGrupo(ctx.chat.id, { antilink: !grupo.antilink });
    return mostrar(ctx, true);
  });

  bot.action('config_reglas', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    const grupo = db.getGrupo(ctx.chat.id);
    const reglas = grupo.rules || t(grupo, 'reglasVacias');
    return ctx.editMessageText(`📜 ${t(grupo, 'reglasTitulo')}\n\n${reglas}`, {
      reply_markup: {
        inline_keyboard: [[{ text: `⚙️ ${t(grupo, 'configTitulo')}`, callback_data: 'menu_config' }]]
      }
    });
  });

  bot.action('config_idioma', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    if (!(await requireAdmin(ctx))) return;
    const grupo = db.getGrupo(ctx.chat.id);
    return ctx.editMessageText(`${t(grupo, 'seleccionarIdioma')}\n\n${t(grupo, 'idiomaActual', { language: grupo.language === 'en' ? 'English' : 'Español' })}`, {
      reply_markup: {
        inline_keyboard: [[{ text: t(grupo, 'espanol'), callback_data: 'config_lang_es' }, { text: t(grupo, 'ingles'), callback_data: 'config_lang_en' }], [{ text: `⚙️ ${t(grupo, 'configTitulo')}`, callback_data: 'menu_config' }]]
      }
    });
  });

  bot.action(/^config_lang_(es|en)$/, async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    if (!(await requireAdmin(ctx))) return;
    db.setGrupo(ctx.chat.id, { language: ctx.match[1] });
    return mostrar(ctx, true);
  });

  bot.action('config_prefijo', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    const grupo = db.getGrupo(ctx.chat.id);
